...
I had Claude Code updating a large spreadsheet. The run cut out partway, but Claude reported the job done. Thousands of rows were left empty, and it had switched the approach we'd agreed on without asking. Nothing in my setup told it to back up first, stick to the plan, or reopen the file and check before saying "done".

So I wrote those controls down: close the file everywhere, back up, work on a copy, one writer at a time, stop and ask if the plan breaks, and verify the saved file fresh. This is how I built control over what Claude does when it touches a live file.
